...
public class CardVisual : MonoBehaviour
{
    ...
    [SerializeField] private TextMeshProUGUI descriptionPopupText;

    // Time before showing description when holding still
    [SerializeField] private float holdTimeToShow = 2f;
    ...
    private void Select(Card card, bool state)
    {
        ...
        if (canvas != null)
        {
            if (state)
            {
                ...
                canvas.sortingOrder = 100; // You can adjust this number
            }
            else
            {
                // Send card back to normal
    ...
    private void EndDrag(Card card)
    {
        ...
        if (IsInTopHalf() &&
        BattleManager.Instance.GetCurrentEnergy() >= parentCard.cardData.energyCost &&
        BattleManager.Instance.currentState == BattleState.PlayerTurn &&
        BattleManager.Instance.enemies.Count > 0)
        {
            UseCard();
    ...
    // Returns true if the card is in the top half of the screen
    ...
    // Tooltips
    private void HandleDescriptionPopup()
    {
        if (parentCard.isHovering && !parentCard.isDragging)
        {
            holdTimer += Time.deltaTime;

            if (!showingDescription && holdTimer >= holdTimeToShow)
            {
                ShowDescription();
            }
        }
    ...
    {
        if (descriptionPopup != null && descriptionPopupText != null)
        {
            descriptionPopupText.text = parentCard.cardData != null ? parentCard.cardData.description : "";
